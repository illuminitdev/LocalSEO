...
function mapReviewsLegacy(reviews = []) {
    return reviews.slice(0, 5).map((r) => ({
...
function normalizeLegacyDetails(details) {
...
async function searchPlacesNew(query) {
    ...
    if (!res.ok) {
        ...
        err.code = data?.error?.status || 'PLACES_NEW_ERROR';
        throw err;
    }
    const places = Array.isArray(data.places) ? data.places : [];
    return places.map(normalizeNewPlace).filter((p) => p?.name);
}
...
async function nearbyCompetitors({ lat, lng, keyword, excludeName }) {
    ...
    try {
        ...
        if (res.ok && Array.isArray(data.places)) {
            return data.places
                ...
                .filter((c) => c.name && (!excludeName || c.name.toLowerCase() !== excludeName.toLowerCase()))
                .slice(0, 5);
        }
    } catch (err) {
