...
function getOAuthClient() {
    ...
    const redirectUri = process.env.GOOGLE_REDIRECT_URI || `${apiBase.replace(/\/$/, '')}/api/integrations/google/callback`;
...
async function getCalendarClient(userId) {
    ...
    const client = getOAuthClient();
    if (!client) return null;
    client.setCredentials({ refresh_token: rows[0].google_refresh_token });
    return { calendar: google.calendar({ version: 'v3', auth: client }), calendarId: rows[0].calendar_id };
}

async function fetchBusyBlocks(userId, timeMin, timeMax) {
...
async function createCalendarEvent(userId, booking, eventType, org) {
    const calClient = await getCalendarClient(userId);
...
async function updateCalendarEvent(userId, googleEventId, booking) {
    const calClient = await getCalendarClient(userId);
...
async function deleteCalendarEvent(userId, googleEventId) {
    ...
    if (!calClient || !googleEventId) return;
    try {
        await calClient.calendar.events.delete({ calendarId: calClient.calendarId, eventId: googleEventId });
    } catch (err) {
