const { verifyToken } = require('../lib/authTokens');
...
async function attachUserFromToken(req, token) {
    ...
    const { rows } = await query(
        `SELECT u.id, u.email, u.name, m.org_id, m.role, o.slug AS org_slug, o.name AS org_name
         FROM users u
...
async function attachOrgBySlug(req, slug) {
...
async function resolveHostUserId(req) {
...
module.exports = { requireAuth, requireHost, authRequired, resolveHostUserId };
